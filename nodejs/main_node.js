...
//    setInterval(update, 1000 / FPS);
    ...
    var draw = {draw: true};
    gui.add(draw, "draw").name('Draw Simulation');
    click.addEventListener("click", function() {
        var socket = io();
        socket.emit('switch simulation', timestep);
        alert(draw.draw);
    });
    ...
    function updateSimulation() {
            ...
            //alert('Send request');
            socket.on('receive simulation data', function(fsd,fsu,fsv){
                //alert("Received data");
                socket.emit('update simulation', timestep);
                updateRender(fsd,fsu,fsv);
            });
    }

    function updateRender(fsd,fsu,fsv) {
